...
class HOME_BUILDER_PT_hidden_header(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if context.space_data.shading.color_type != 'OBJECT':
            ...
            box.operator("blendertomob.set_recommended_settings",
        ...
        if not use_hud:
            ...
            row.operator("blendertomob.scene_navigator", text="", icon='MENU_PANEL')
...
class HOME_BUILDER_PT_project_rooms(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if len(room_scenes) > 1:
            ...
            button_col.operator("blendertomob.move_room_scene", text="", icon='TRIA_UP').move_up = True
            button_col.operator("blendertomob.move_room_scene", text="", icon='TRIA_DOWN').move_up = False
        ...
        for scene in room_scenes:
            ...
            # Switch button
            op = row.operator("blendertomob.switch_room", text=scene.name, icon=icon)
            op.scene_name = scene.name
            
            # Delete button (only if more than one room)
            if len(room_scenes) > 1:
                op = row.operator("blendertomob.delete_room", text="", icon='X')
                op.scene_name = scene.name
        
        # Room management buttons
        list_col.separator()
        row = list_col.row(align=True)
        row.operator("blendertomob.create_room", text="Add", icon='ADD')
        
        # Only show these if not in a layout view
        if not context.scene.get('IS_LAYOUT_VIEW'):
            row.operator("blendertomob.rename_room", text="Rename", icon='GREASEPENCIL')
            row.operator("blendertomob.duplicate_room", text="Duplicate", icon='DUPLICATE')
        ...
        if not other_rooms:
            ...
            for room_scene in other_rooms:
                ...
                op = row.operator('blendertomob.toggle_link_room', text='', icon=icon, depress=is_linked)
                ...
                if is_linked and linked_obj:
                    ...
                    for cat_key, cat_label in [('walls', 'Walls'), ('lights', 'Lights'), ('products', 'Products')]:
                        ...
                        op = cat_row.operator('blendertomob.toggle_linked_room_category', text=cat_label, icon=cat_icon)
...
class HOME_BUILDER_MT_room_list(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.operator("blendertomob.create_room", text="Create Room", icon='ADD')
        layout.operator("blendertomob.rename_room", text="Rename Room", icon='GREASEPENCIL')
        op = layout.operator("blendertomob.delete_room", text="Delete Room", icon='X')
...
class HOME_BUILDER_PT_room_layout_reference_image(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if has_image:
            col = layout.column(align=True)
            col.scale_y = 1.3
            col.operator("blendertomob.set_scale_with_two_points", 
