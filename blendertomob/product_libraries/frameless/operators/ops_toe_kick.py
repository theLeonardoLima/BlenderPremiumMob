...
class hb_frameless_OT_create_toe_kick_detail(bpy.types.Operator):
    ...
    def _draw_cabinet_side_detail(self, context, scene, props):
        ...
        # Position the detail so the floor is at origin (0,0)
        # -X axis goes toward the back (depth), +Y axis goes up (height)
        # Origin (0,0) is at the floor level at the front of the cabinet side panel
        ...
        side_profile.set_point(0, Vector((0, toe_kick_height + corner_size, 0)))
        # Go down to bottom-front corner of cabinet side
        side_profile.add_point(Vector((0, toe_kick_height, 0)))
        ...
        bottom_panel.set_point(0, Vector((0, toe_kick_height + part_thickness, 0)))
        bottom_panel.add_point(Vector((-corner_size, toe_kick_height + part_thickness, 0)))
        ...
        door_profile.set_point(0, Vector((door_to_cab_gap, toe_kick_height + corner_size, 0)))
        door_profile.add_point(Vector((door_to_cab_gap, toe_kick_height + part_thickness - door_overlay, 0)))
        door_profile.add_point(Vector((door_to_cab_gap + door_thickness, toe_kick_height + part_thickness - door_overlay, 0)))
        door_profile.add_point(Vector((door_to_cab_gap + door_thickness, toe_kick_height + corner_size, 0)))
        ...
        floor_line.set_point(0, Vector((detail_left, 0, 0)))
        floor_line.add_point(Vector((detail_right, 0, 0)))
        ...
        floor_text.set_location(Vector((detail_right + units.inch(0.25), 0, 0)))
        ...
        tk_dim.obj.location = Vector((-corner_size - units.inch(0.5), 0, 0))
        ...
        door_mid_y = toe_kick_height + (part_thickness - door_overlay + corner_size) / 2
...
class hb_frameless_OT_assign_toe_kick_to_cabinets(bpy.types.Operator):
    """Assign the selected toe kick detail to selected base and tall cabinets"""
    bl_idname = "hb_frameless.assign_toe_kick_to_cabinets"
    bl_label = "Assign Toe Kick to Cabinets"
    bl_description = "Create toe kick molding extrusions on selected base and tall cabinets using the active toe kick detail"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        if not profiles:
            self.report({'WARNING'}, "No molding profiles or solid lumber found in toe kick detail")
            return {'CANCELLED'}
        
        # Collect unique BASE and TALL cabinets from selection
        cabinets = []
        for obj in context.selected_objects:
            cabinet_bp = None
            if obj.get('IS_CABINET_BP') or obj.get('IS_FRAMELESS_CABINET_CAGE'):
            ...
            if cabinet_bp and cabinet_bp not in cabinets:
                ...
                if cab_type in ('BASE', 'TALL'):
                    cabinets.append(cabinet_bp)
        
        if not cabinets:
            self.report({'WARNING'}, "No valid base or tall cabinets selected")
            return {'CANCELLED'}
        
        # Remove any existing toe kick molding on selected cabinets
        for cabinet in cabinets:
            self._remove_existing_toe_kick(cabinet)
            cabinet['TOE_KICK_DETAIL_NAME'] = toe_kick.name
            cabinet['TOE_KICK_DETAIL_SCENE'] = toe_kick.detail_scene_name
        ...
        # Create toe kick molding for each group
    ...
    def _remove_existing_toe_kick(self, cabinet):
        """Remove any existing toe kick molding children from the cabinet."""
        children_to_remove = []
        for child in cabinet.children:
            if child.get('IS_TOE_KICK_MOLDING') or child.get('IS_TOE_KICK_PROFILE_COPY'):
    ...
    def _find_adjacent_cabinet(self, cabinet, side, all_cabinets, tolerance=0.02):
        """Find a BASE or TALL cabinet adjacent to the given side."""
        bounds = self._get_cabinet_bounds(cabinet)
        axis = self._get_wall_direction(cabinet)
        ...
        for other in all_cabinets:
            ...
            if other_type not in ('BASE', 'TALL'):
                continue
    ...
    def _create_toe_kick_for_group(self, context, group, profile, walls, all_cabinets, target_scene):
        """Create toe kick molding extrusion for a group of cabinets."""
        ...
        profile_copy.name = f"TK_Profile_{profile.name}"
        profile_copy['IS_TOE_KICK_PROFILE_COPY'] = True
        ...
        # Create the curve
        curve_data = bpy.data.curves.new(name=f"TK_Path_{profile.name}", type='CURVE')
        ...
        tk_obj = bpy.data.objects.new(f"Toe_Kick_{profile.name}", curve_data)
        target_scene.collection.objects.link(tk_obj)
        
        # Parent to first cabinet - position at floor level (Z=0 in world, bottom of cabinet)
        tk_obj.parent = first_cab
        tk_obj.location = (0, 0, -first_bounds['bottom_z'] + first_cab.matrix_world.translation.z + profile_offset_y)
        # Simplified: toe kick sits at floor, so Z offset relative to cabinet origin
        # The cabinet bottom_z in world space tells us where the floor is
        # We need the local Z that corresponds to the floor
        floor_z_local = (first_cab.matrix_world.inverted() @ Vector((0, 0, first_bounds['bottom_z']))).z
        tk_obj.location = (0, 0, floor_z_local + profile_offset_y)
        tk_obj['IS_TOE_KICK_MOLDING'] = True
        tk_obj['TK_PROFILE_NAME'] = profile.name
        
        # Add Smooth by Angle modifier
        smooth_mod = tk_obj.modifiers.new(name="Smooth by Angle", type='NODES')
        ...
        if props.cabinet_styles and style_index < len(props.cabinet_styles):
            ...
            if material:
                if len(tk_obj.data.materials) == 0:
                    tk_obj.data.materials.append(material)
                else:
                    tk_obj.data.materials[0] = material
...
classes = (
    ...
    hb_frameless_OT_assign_toe_kick_to_cabinets,
)
