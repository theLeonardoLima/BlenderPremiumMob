...
| `A011` | **CRITICAL** | API Registration | `bpy.utils.unregister_class(cls)` failed on re-imported module instances because `cls` was a new Python object while Blender's RNA held the previous class instance. Resolved via `getattr(bpy.types, cls.__name__)` lookup. | [hb_assets.py](file:///home/theleoinfo/www/BlenderToMob/hb_assets.py#L404-L425), [blendertomob/hb_assets.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/blendertomob/hb_assets.py#L404-L425) |
| `A012` | **CRITICAL** | API Registration | Renamed `HB_AssetLibraryEntry` to `BTM_AssetLibraryEntry` to isolate from legacy Home Builder add-ons, and implemented idempotent `if hasattr(bpy.types, cls.__name__): continue` registration guard to eliminate subclass collisions. | [hb_assets.py](file:///home/theleoinfo/www/BlenderToMob/hb_assets.py#L248-L415), [blendertomob/hb_assets.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/hb_assets.py#L248-L415) |
| `A013` | **HIGH** | Extension Hot-Reload | Added `HB_AssetLibraryEntry = BTM_AssetLibraryEntry` alias to guarantee seamless compatibility when Blender reloads extension modules from `sys.modules` cache without restarting Blender. | [hb_assets.py](file:///home/theleoinfo/www/BlenderToMob/hb_assets.py#L265), [blendertomob/hb_assets.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/hb_assets.py#L265) |
| `A014` | **CRITICAL** | Extension Packaging | Fixed `build.py` zip relative path archive structure (`os.path.relpath(file_path, source_dir)`). Previously nested files inside a redundant `blendertomob/` subfolder within `blendertomob.zip`, causing Blender 4.2+ extension installer to import incorrect root module paths. | [build.py](file:///home/theleoinfo/www/BlenderToMob/build.py#L28) |
| `A015` | **CRITICAL** | Module In-Memory Cache | Added `importlib.reload` loop for `sys.modules` and dynamic `getattr(hb_assets, 'BTM_AssetLibraryEntry', getattr(hb_assets, 'HB_AssetLibraryEntry', None))` in `__init__.py` to eliminate `AttributeError` when Blender re-installs extensions in an active session. | [__init__.py](file:///home/theleoinfo/www/BlenderToMob/__init__.py#L30-L40), [blendertomob/__init__.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/__init__.py#L48-L58) |
| `A017` | **CRITICAL** | API & RNA Lifecycle | Added `unregister_properties()` prior to class unregistration/registration in `data/properties.py` to unbind `bpy.types.Object` and `bpy.types.Scene` PointerProperties. Prevents Blender's C/RNA engine from locking `BTM_PG_WallSegment` in memory and throwing `register_class(...): already registered as a subclass 'BTM_PG_WallSegment'`. Cleaned type annotations and synchronized root/submodule files. | [blendertomob/data/properties.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/data/properties.py#L525-L570), [blendertomob/hb_props.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/hb_props.py#L460-L498), [blendertomob/operators/walls.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/operators/walls.py#L5730-L5750) |
| `A018` | **HIGH** | Extension Hot-Reload & RNA Hygiene | Replaced legacy `register_classes_factory` with safe `getattr(bpy.types, cls.__name__)` unregister/register pattern across 66 modules in `product_libraries` (frameless, closets, face_frame) and root `ops.py` / `doors_windows.py`. Completely eliminates registration collisions upon extension updates or re-installs. | [ops.py](file:///home/theleoinfo/www/BlenderToMob/ops.py#L662-L683), [operators/doors_windows.py](file:///home/theleoinfo/www/BlenderToMob/operators/doors_windows.py#L1445-L1467) |
| `A019` | **MEDIUM** | PEP 526 Type Compliance | Converted 910 Blender property declarations across 75 files from invalid type annotation position (`name: bpy.props.StringProperty(...)`) to standard assignment syntax (`name = bpy.props.StringProperty(...)`), eliminating Pyright / IDE static type checker warnings. | [ops.py](file:///home/theleoinfo/www/BlenderToMob/ops.py#L35-L57), [hb_props.py](file:///home/theleoinfo/www/BlenderToMob/hb_props.py#L330-L340) |
| `A020` | **MEDIUM** | Python Code Quality & Module Sync | Replaced 78 bare `except:` statements with explicit `except Exception:` following Python quality standards (`python-skills/code-quality/SKILL.md`) to prevent swallowing system signals. Synchronized root files with `blendertomob/` extension sources. | [hb_detail_library.py](file:///home/theleoinfo/www/BlenderToMob/hb_detail_library.py#L32), [blendertomob/hb_layouts.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/hb_layouts.py#L1940) |
| `A021` | **LOW** | Blender Toolkit & Compat Integration | Integrated 17 skills from `kevinbadi/blender-skills` (`.agents/skills/blender-toolkit`, `turntable`, `polyhaven-studio-setup`, etc.) into repository agent skills. Verified `compat.py` for GPU shader version fallbacks and Geometry Nodes socket RNA access in Blender 5.2.0 LTS. | [compat.py](file:///home/theleoinfo/www/BlenderToMob/blendertomob/compat.py#L90-L100), [.agents/skills/blender-toolkit/SKILL.md](file:///home/theleoinfo/www/BlenderToMob/.agents/skills/blender-toolkit/SKILL.md) |
| `A022` | **HIGH** | IDE Type Resolution & Code Quality | Installed `fake-bpy-module-latest` in local site-packages and created `pyrightconfig.json` + `pyproject.toml`. Resolved all IDE "Cannot find module `bpy`" errors, formatted code with `ruff`, and verified 100% clean compilation across all 265 Python files. | [pyrightconfig.json](file:///home/theleoinfo/www/BlenderToMob/pyrightconfig.json#L1-L12), [pyproject.toml](file:///home/theleoinfo/www/BlenderToMob/pyproject.toml#L1-L15) |
